import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { FileText, Phone, Mail } from 'lucide-react';
import { useTranslation } from '@/hooks/useTranslation';

interface ConditionsGeneralesPageProps {
  onNavigate: (page: string) => void;
}

export function ConditionsGeneralesPage({ onNavigate }: ConditionsGeneralesPageProps) {
  const { t } = useTranslation();

  return (
    <div className="min-h-screen bg-gradient-surface">
      {/* Hero Section */}
      <section className="bg-gradient-hero text-center py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-6">
            {t('terms.title')}
          </h1>
          <p className="text-xl text-white/90 mb-8">
            {t('terms.subtitle')}
          </p>
        </div>
      </section>

      <div className="max-w-4xl mx-auto px-6 py-16">
        {/* Informations légales */}
        <Card className="mb-8">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <FileText className="h-5 w-5 text-primary" />
              <span>{t('terms.legalInfo')}</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-medium mb-2">{t('terms.company')}</h4>
                <p className="text-sm text-muted-foreground">
                  {t('terms.companyDescription')}<br/>
                  {t('terms.siret')}<br/>
                  {t('terms.transportNumber')}<br/>
                  {t('terms.insurance')}
                </p>
              </div>
              <div>
                <h4 className="font-medium mb-2">{t('terms.headquartersTitle')}</h4>
                <p className="text-sm text-muted-foreground">
                  {t('terms.address')}<br/>
                  {t('terms.cityPostal')}<br/>
                  {t('terms.phone')}<br/>
                  {t('terms.email')}
                </p>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Conditions générales */}
        <div className="space-y-6">
          {Object.entries(t('terms.sections')).map(([key, section]: [string, any], index) => (
            <Card key={index}>
              <CardHeader>
                <CardTitle className="text-lg">{section.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-muted-foreground whitespace-pre-line">
                  {section.content}
                </div>
              </CardContent>
            </Card>
          ))}
        </div>

        {/* Dernière mise à jour */}
        <Card className="mt-8">
          <CardContent className="pt-6">
            <div className="text-center text-sm text-muted-foreground">
              <p>{t('terms.lastUpdate')}</p>
              <p className="mt-2">
                {t('terms.updateNote')}
              </p>
            </div>
          </CardContent>
        </Card>

        {/* Contact pour questions */}
        <Card className="mt-8 bg-gradient-hero">
          <CardContent className="pt-6 text-center">
            <h3 className="text-xl font-bold text-white mb-4">
              {t('terms.questionsTitle')}
            </h3>
            <p className="text-white/90 mb-6">
              {t('terms.questionsDescription')}
            </p>
            <div className="space-x-4">
              <Button 
                onClick={() => onNavigate('contact')}
                className="bg-white text-primary hover:bg-white/90"
              >
                <Phone className="h-4 w-4 mr-2" />
                {t('terms.contactUs')}
              </Button>
              <Button 
                variant="outline"
                className="border-white text-white hover:bg-white/10"
              >
                <Mail className="h-4 w-4 mr-2" />
                {t('terms.legalEmail')}
              </Button>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}